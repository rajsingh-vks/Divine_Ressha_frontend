...
async function proxy(request: Request, addressId: string) {
  ...
  for (const path of ADDRESS_PATHS) {
    const response = await fetch(`${BACKEND_API_URL}${path}/${encodeURIComponent(addressId)}`, {
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach address service.' }, { status: 502 });
  }
