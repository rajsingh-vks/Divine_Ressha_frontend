...
async function proxy(request: Request) {
  ...
  const candidatePaths = ['/auth/mobile-login/initiate', '/api/auth/mobile-login/initiate'];
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach mobile login service.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { detail: error instanceof Error ? error.message : 'Unable to reach mobile login service.' },
      { status: 500 }
    );
  }
}
