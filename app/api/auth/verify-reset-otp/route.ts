...
async function proxy(request: Request) {
  ...
  const candidatePaths = ['/auth/verify-reset-otp', '/api/auth/verify-reset-otp'];
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach password verification service.' }, { status: 502 });
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { detail: error instanceof Error ? error.message : 'Unable to reach password verification service.' },
      { status: 500 }
    );
  }
}
