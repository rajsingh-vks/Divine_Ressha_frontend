...
async function proxy(request: Request, orderId: string) {
  ...
  for (const path of ORDER_PATHS) {
    const response = await fetch(`${BACKEND_API_URL}${path}/${encodeURIComponent(orderId)}/send-confirmation`, {
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach send confirmation service.' }, { status: 502 });
  }
