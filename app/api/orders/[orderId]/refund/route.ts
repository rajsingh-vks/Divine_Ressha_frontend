...
async function proxy(request: Request, orderId: string) {
  ...
  for (const path of ORDER_PATHS) {
    const response = await fetch(`${BACKEND_API_URL}${path}/${encodeURIComponent(orderId)}/refund`, {
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach order refund service.' }, { status: 502 });
  }
