...
async function proxy(request: Request, orderId: string) {
  ...
  for (const path of ORDER_PATHS) {
    const response = await fetch(`${BACKEND_API_URL}${path}/${encodeURIComponent(orderId)}/cancel`, {
      method: 'PATCH',
      headers,
      body,
      cache: 'no-store',
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach order cancel service.' }, { status: 502 });
  }
