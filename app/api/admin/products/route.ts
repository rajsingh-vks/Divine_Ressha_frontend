...
const COLLECTION_PATHS = ['/products', '/api/products'];
...
async function proxy(request: Request) {
  ...
  for (const basePath of COLLECTION_PATHS) {
    const url = `${BACKEND_API_URL}${basePath}${query}`;
  ...
  if (!backendResponse) {
    return NextResponse.json({ detail: 'Unable to reach products service.' }, { status: 502 });
  }
